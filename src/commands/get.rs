...
pub fn parse(args: Vec<String>) -> Result<Request, RedisProtocolError> {
    ...
    let key = args.first().expect("Failed to fetch first element of array");
    Ok(Request::GET { key: String::from(key) })
}
...
pub fn default_handle(args: Request) -> Result<OwnedFrame, RedisProtocolError> {
    ...
    let default_values: HashMap<String, String> = HashMap::from([
...
/// not inside `values`
pub fn handle<V: AsFrame+ Clone>(values: &HashMap<String, V>, args: Request) -> Result<OwnedFrame, RedisProtocolError> {
